  ...
  const reviews = [
    {
      ...
      text:
          "Замовив в BureauX розробку дизайну нашого ресторану і щиро задоволений результатом. Вони вміють дійсно злитися з бажаннями клієнта і додали веселого колориту та стильного оформлення до нашого приміщення. Висока якість та велике професійне знання!",
    },
    {
      image: review1Img,
      name: "Оксана Т.",
      ...
      nickName: "oksana1986",
      text:
          "Дякую BureauX за стильний та сучасний дизайн мого будинку! Вони допомогли перетворити простір у гармонійне та функціональне житло. Завдяки ним моя мрія про ідеальний дім збулася! Рекомендую їх як надійного та талановитого партнера у справі створення унікальних інтер'єрів.",
    },
  ];
      ...
      function handleResize() {
        ...
        if (newWindowWidth <= 480) {
          ...
          maxLogosInRow = Math.floor(
              (containerWidth - titleWidth) / LOGO_WIDTHS.small
          );
          ...
          const titleWidth = titleRef.current.getBoundingClientRect().width;
          maxLogosInRow = Math.floor(
              (containerWidth - titleWidth) / LOGO_WIDTHS.medium
          );
          ...
          const titleWidth = titleRef.current.getBoundingClientRect().width;
          maxLogosInRow = Math.floor(
              (containerWidth - titleWidth) / LOGO_WIDTHS.large
          );
